...
def create_newsletter():
    ...
    import copy
    import io
    clean_slide = prs.slides[9]
    clean_bg_xml = copy.deepcopy(clean_slide.element.xpath('./p:cSld/p:bg'))
    clean_shapes = []
    for shp in clean_slide.shapes:
        if shp.shape_type == 13: # PICTURE
            clean_shapes.append(('picture', shp.image.blob, shp.left, shp.top, shp.width, shp.height))
        else:
            clean_shapes.append(('other', copy.deepcopy(shp.element)))
    ...
    def ensure_slide_space(needed: float) -> bool:
        ...
        if current_y > max_height - needed:
            ...
            if slide_index < len(content_slides):
                ...
                if clean_bg_xml:
                    new_slide.element.cSld.insert(0, copy.deepcopy(clean_bg_xml[0]))
                
                for item in clean_shapes:
                    if item[0] == 'picture':
                        _, blob, left, top, width, height = item
                        stream = io.BytesIO(blob)
                        new_slide.shapes.add_picture(stream, left, top, width, height)
                    else:
                        _, el = item
    ...
    for sheet_name, df_sheet in dfs.items():
        ...
        for index, row in df_sheet.iterrows():
            ...
            # ── All other sheets: original sequential rendering ────────────
                ...
                paragraphs_to_print = []
                paragraphs_to_print.append(('title', title, pdf_url))
                ...
                while paragraphs_to_print:
                    ...
                    while paragraphs_to_print:
                        ...
                        if current_y + box_height + para_height > max_height and not first_para:
                            break
    ...
    for i, slide in enumerate(prs.slides):
        for shape in slide.shapes:
            if shape.has_text_frame:
                for paragraph in shape.text_frame.paragraphs:
                    for run in paragraph.runs:
                        if re.search(r'Page\s+\d+\s+of\s+\d+', run.text, re.IGNORECASE):
                            run.text = re.sub(r'Page\s+\d+\s+of\s+\d+', f'Page {i+1} of {total_slides}', run.text, flags=re.IGNORECASE)
